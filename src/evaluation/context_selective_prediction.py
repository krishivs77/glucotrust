...
PREDICTIONS_PATH = Path("reports/context_xgb_ensemble_uncertainty_predictions.csv")
OUT_PATH = Path("reports/context_selective_prediction_results.csv")
...
def selective_prediction_for_target(pred_df: pd.DataFrame, target: str) -> pd.DataFrame:
    ...
    # Lower ensemble standard deviation means higher model confidence.
...
def main() -> None:
    if not PREDICTIONS_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run src/models/train_context_xgb_ensemble_uncertainty.py first."
        )
    ...
    for target in sorted(pred_df["target"].unique()):
        print(f"Running context selective prediction analysis for {target}")
    ...
    print()
    print(f"Saved context selective prediction results to {OUT_PATH}")
    print()
    print(results)
